...
function PainelContato({ detail, atd, config, chatId, setToast, reload, assistantName, onBack, username, me }) {
...
function Detalhe({ chatId, me, status, assistantName, config, setToast, go, users, onListChanged, painelAberto, setPainelAberto, setMobileView, baseRoute }) {
  ...
  return html`<${Fragment}>
    ...
    <aside class=${`atd-panel${painelAberto ? '' : ' collapsed'}`}>
      ${painelAberto ? html`<${PainelContato} detail=${detail} atd=${atd} config=${config} chatId=${chatId} setToast=${setToast} reload=${() => resource.reload()} assistantName=${assistantName} onBack=${() => setMobileView('conversa')} username=${me && me.username} me=${me}/>` : null}
    </aside>
  </${Fragment}>`;
}
